...
#if defined(Q_OS_WIN)
#if !defined(ZSHOT_DEBUG_CAPTURE)
...
// LINUX
#else
// Call cmake with -DZSHOT_DEBUG_CAPTURE=ON to enable easier debugging
#if !defined(ZSHOT_DEBUG_CAPTURE)
...
CaptureWidget::~CaptureWidget()
{
    ...
    if (m_captureDone) {
        ...
        Zshot::instance()->exportCapture(
          pixmap(), geometry, m_context.request);
    } else {
        emit Zshot::instance()->captureFailed();
    }
}
...
void CaptureWidget::paintEvent(QPaintEvent* paintEvent)
{
    ...
    */
    ...
    if (!isActiveWindow()) {
        drawErrorMessage(
          tr("Zshot has lost focus. Keyboard shortcuts won't "
        ...
        drawErrorMessage(tr("Configuration error resolved. Launch `zshot "
