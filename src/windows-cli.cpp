...
void CallZshot(const std::wstring args, bool wait)
{
    // generate full path for zshot executable
...
// Console 'wrapper' for zshot on windows
int wmain(int argc, wchar_t* argv[])
{
    // if no args, do not wait for stdout
    if (argc == 1) {
        std::cout << "Starting zshot in daemon mode" << std::endl;
        CallZshot(L"", false);
    } else {
        std::wstring argString = joinArgs(argc, argv);
        CallZshot(argString, true);
    }
    std::cout.flush();
    return 0;
}
